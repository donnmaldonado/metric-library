...
| Path | Contents |
|---|---|
| `data/metrics.json` | **Source of truth.** Per metric: metricId, label, tier, vertical, industry, description, SQL, YAML, numerator/denominator, dimensions, data sources, parent/child/correlated metrics and `formulaInputs`. |
...
- `customer_health_score` and `vendor_scorecard_rating`: the composite weights are placeholders (0.4/0.3/0.3 and equal weights). `customer_health_score` could also merge into `account_health_score`.
...
- `payback_ratio`: the description says LTV / CAC, the numerator/denominator say CAC payback. It's currently an alias of `cac_payback`. Keep it, re-point it to `ltv_cac`, or retire it.
...
- `average_daily_attendance`: implemented as an attendance rate; in school finance, ADA is usually a count (`days_attended / school_days`).
...
- Labels to fix: `customer_ltv` → "Realized Customer LTV", `backlink_count` → "Referring Domains", `backlinks_count` → "Backlinks".
